...
---
# 35届CSP：补丁应用
1. **预处理**：过滤 `#` 开头的注释。按 `@@` 分割出块，正则解析 `NN, MM, mm`，并根据每行首位符号提取**原文片段**与**新文片段**。
2. **合法性校验**：检查 `NN` 是否递增、片段行数是否匹配 `MM/mm`、格式是否正确。
3. **动态匹配**：在原文件 $[NN-(MM-1), NN+(MM-1)]$ 范围内寻找内容匹配且不与前块重叠的偏移 ，按**绝对值最小、数值最小**原则取舍。
4. **应用替换**：执行替换后，实时累加 $\delta$ 和长度变化量 $\Delta = mm - MM$ 以修正后续块的搜索基准。
---
# 32届CSP：树上搜索
1. 构建树形结构，计算每个节点的子树总权重，并记录其所有后代节点集合。
2. 在当前活跃节点集中，寻找使 $w_\delta = |W_{total} - 2W_{sub\_active}|$ 最小（且编号最小）的节点作为问题。
3. 根据目标类别是否在提问节点的后代集中，判断用户回答为“是”或“否”，据此保留或剔除对应子树，缩小活跃范围。
4. 重复上述过程直至活跃集合仅剩一个节点，输出询问序列。
